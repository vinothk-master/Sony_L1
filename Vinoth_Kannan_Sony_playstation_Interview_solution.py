...
def evaluate(expression: str) -> Optional[int]:
    ...
    if parentheses_stack:
        return 0
    def helper(op, cur): #This helper function is used to perform the calculation based on the operation in the string
        if op =="+":             
            stack.append(cur)  
        elif op =="-":
            stack.append(-cur) 
        elif op =="*":
            stack.append(stack.pop()*cur)  #According to the 'BODMAS' rule * has the higher priority than "+-", so the * is performed with the top value int the stack
        elif op =="/":
            stack[-1] = int(float(stack[-1]) / cur) #According to the 'BODMAS' rule / has the higher priority than "*+-" so the / is performed with the top value in the stack and appended
    for char in range(len(expression)):

        if expression[char].isdigit():
            cur = cur*10+int(expression[char]) # The digit can be of multiple digit places, inorder to validate as a single number we use this
        elif expression[char]=='(': #if we see this in the string, we shall append to my stack along with the operation to perform
            stack.append(op)
            cur =0
            op="+"
        elif expression[char] in ['+','-','*','/',')']:
            helper(op, cur)
            if expression[char]==')': #if we see in the string which means, we need to perform the calculation within the ()
                cur =0
